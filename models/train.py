...
class TrainModel:
        ...
        for epoch in range(num_epoch):
            ...
            for i, (images, masks) in progress_bar:
                ...
                progress_bar.set_postfix(
                    dice_loss=round(total_train_dice_loss.data.item() / (i + 1), 4),
                    dice_coeff=round(
                        1 - (total_train_dice_loss.data.item() / (i + 1)), 4
                    ),
                )
                progress_bar.update()
            ...
            with torch.no_grad():
                ...
                for i, (images, masks) in eval_progress_bar:
                    ...
                    eval_progress_bar.set_postfix(
                        avg_dice_val_loss=round(
                            total_test_dice_loss.data.item() / test_step, 4
                        ),
                        avg_val_dice_coeff=round(
                            1 - (total_test_dice_loss.data.item() / test_step), 4
                        ),
                    )
            ...
            history["training_dice_loss"].append(
                round(total_train_dice_loss.data.item() / train_step, 4)
            )
            history["test_dice_loss"].append(
                round(total_test_dice_loss.data.item() / test_step, 4)
            )
    ...
    def early_stopping(self, model, val_loss, epoch, patience=10, min_delta=0.01):
        ...
        if (
            ...
            and self.CURRENT_VAL_LOSS - val_loss >= min_delta
        ):
            # Val loss improved -> save model
            print(
                f"[Info] Val loss improved from {self.CURRENT_VAL_LOSS} to {val_loss}"
            )
    ...
    def save_model(self, model):
        ...
        model_name = f"elunet_symptom_lara_leaf_{date_postfix}.pth"
